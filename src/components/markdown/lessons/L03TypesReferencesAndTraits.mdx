...
One provides read only access and many of these references may coexist. One
provides write access and if it exists then it must be the only reference to a
value.

If a reference to a value exists, we say that the reference borrows the value.

| Type     | Value               | Read | Write | Drop T | Create `&T` | Create `&mut T` |
| :------- | :------------------ | :--- | :---- | :----- | :---------- | :-------------- |
| `T`      | Owned               | Yes* | Yes*  | Yes*   | Yes†        | Yes†            |
| `&T`     | Borrowed            | Yes  | No    | No     | Yes         | No              |
| `&mut T` | Borrowed as mutable | Yes  | Yes   | No     | Yes‡        | No              |

*\* only if no borrows exist.*

*† only if no mutable borrow exists.*

*‡ only by downgrading and discarding the mutable borrow.*

In summary, a value can be borrowed by any number of immutable references, **or**
can be borrowed as mutable by at most one mutable reference.

As an additional note, because any number of immutable references to the same
value can exist, `&T` behaves with copy semantics regardless of `T`. Conversely,
`&mut T` always has move semantics (moving the borrow, not the value).

It is enforced that a reference cannot outlive the value it borrows, as this
would allow a use after free memory error. Equivalently, a value cannot be
dropped while it is borrowed, and if a values owner goes out of scope while a
borrow exists the program will not compile. This is called lifetime checking,
and all references have a lifetime (which the value cannot be dropped or moved
durin).
