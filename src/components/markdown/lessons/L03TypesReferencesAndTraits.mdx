...
You will recall from last lesson thay memory safety requires that all memory
accessed
...
| `char` | 4 | Any value in the `0x0000` to `0xD7FF` or `0xE000` to `0x10FFFF` range |
...
*\*the `usize` and `isize` typed have a platform specific size. For a 32 bit
...
are the two with zero size. `()` is pronounced "unit" and is the type with only
...
may place padding between them according to arcane rules known as the ["default
representation"][repr]. `(i32, char)` is a sequence of a 32-bit integer and a 
unicode scalar character (UTF-32). You can access the parts of a tuple through
numeric field access starting at 0. You can also destructure tuples, but again
