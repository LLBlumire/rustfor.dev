...
In order to do this, the Rust compiler meticulously tracks and enforces when
all memory is acquired and released, when all values are created and destroyed.
...
Scopes, tautologically for the purpose of our explanation here, is simply
...
So, values are in scope from when they are created, until the end of the block
...
indirectiom, and have the `"Hello"` value be allocated in some other location,
